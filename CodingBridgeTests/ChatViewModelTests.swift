...
final class ChatViewModelTests: XCTestCase {
    ...
    func test_parseJSONLToMessages_parsesMessagesArray() {
        let (viewModel, _, _, _) = makeFixture()
        let json = """
        {
          "messages": [
        ...
        let messages = viewModel.parseJSONLToMessages(json)
    ...
    func test_parseExportMessage_toolUseSerializesInput() {
        let (viewModel, _, _, _) = makeFixture()
        let json: [String: Any] = [
            "type": "tool_use",
            "name": "Shell",
        ...
        let messages = viewModel.parseExportMessage(json)

        XCTAssertEqual(messages.count, 1)
        XCTAssertEqual(messages.first?.role, .toolUse)
        XCTAssertTrue(messages.first?.content.hasPrefix("Shell(") == true)
        XCTAssertTrue(messages.first?.content.contains("\"cmd\"") == true)
    }

    func test_parseExportMessage_toolResultErrorMapsToError() {
        let (viewModel, _, _, _) = makeFixture()
        let json: [String: Any] = [
            "type": "tool_result",
            "output": "failed",
    ...
    func test_parseExportMessage_toolResultSuccessMapsToToolResult() {
        let (viewModel, _, _, _) = makeFixture()
        let json: [String: Any] = [
            "type": "tool_result",
            "output": "ok",
        ...
        XCTAssertEqual(messages.count, 1)
        XCTAssertEqual(messages.first?.role, .toolResult)
    }

    func test_parseExportMessage_thinkingEmptyReturnsEmpty() {
        let (viewModel, _, _, _) = makeFixture()
        let json: [String: Any] = [
            "type": "thinking",
            "content": ""
    ...
    func test_parseExportMessage_userSkipsMeta() {
        let (viewModel, _, _, _) = makeFixture()
        let json: [String: Any] = [
            "type": "user",
            "isMeta": true,
    ...
    func test_parseExportMessage_userSkipsCaveat() {
        let (viewModel, _, _, _) = makeFixture()
        let json: [String: Any] = [
            "type": "assistant",
            "content": "Caveat: The messages below were generated by Claude"
        ]
    ...
    func test_parseExportMessage_structuredContentParsesText() {
        let (viewModel, _, _, _) = makeFixture()
        let json: [String: Any] = [
            "type": "assistant",
            "content": [
    ...
    func test_parseLocalCommandXML_parsesCommand() {
        let (viewModel, _, _, _) = makeFixture()
        let content = "<command-name>git</command-name><command-args>status</command-args><local-command-stdout>clean</local-command-stdout>"

        let command = viewModel.parseLocalCommandXML(content)

        XCTAssertEqual(command?.name, "git")
        XCTAssertEqual(command?.args, "status")
        XCTAssertEqual(command?.stdout, "clean")
    }

    func test_parseLocalCommandXML_missingNameReturnsNil() {
        let (viewModel, _, _, _) = makeFixture()
        let content = "<command-args>status</command-args>"
    ...
    func test_parseExportMessage_localCommandXMLConvertsToSystemMessage() {
        let (viewModel, _, _, _) = makeFixture()
        let content = "<command-name>git</command-name><command-args>status</command-args><local-command-stdout>clean</local-command-stdout>"
        let json: [String: Any] = [
            "type": "assistant",
        ...
        XCTAssertTrue(messages.first?.content.contains("git status") == true)
        XCTAssertTrue(messages.first?.content.contains("clean") == true)
    }
